document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- VERPLAATSTE HULPFUNCTIES (NU BOVENAAN) ---
    ...
    // --- EINDE VERPLAATSTE HULPFUNCTIES ---
    ...
    async function initializeApp() {
        ...
        // 3. Controleer Selectors (NU is checkSelectors gedefinieerd)
        if (!checkSelectors()) {
    ...
    // --- Alle overige functies (formatCurrency, populateDateDropdowns, ...) ---
    // Deze blijven hieronder, ongewijzigd van Versie 5
    // ... (copy/paste alle functies vanaf formatCurrency tot en met generateBreakdown uit de vorige versie) ...
     ...
     function populateDateDropdowns() {
        if (!inputs?.p1?.birthYear || !inputs?.p2?.birthYear) { 
             console.error("Inputs object (birthYear) not available in populateDateDropdowns");
             return;
        }
    ...
    function calculateFrance(vals) {
        ...
        if (isPensionerHousehold) {
             const aantalPensioners = partners.filter(p => {
                 ...
                 const aowMoment = new Date(p.birthYear + aowInfo.years, (p.birthMonth || 1) - 1 + aowInfo.months);
             ...
             if (incomeBaseForAbattement <= drempel1 * aantalPensioners) { abattement65plus = aftrek1 * aantalPensioners; } 
             else if (incomeBaseForAbattement <= drempel2 * aantalPensioners) { abattement65plus = aftrek2 * aantalPensioners; }
        }
        ...
        let tax = belastingPerPart * parts;
         ...
         if (childParts > 0 && baseParts > 0) { 
             ...
             if (currentAdvantage > maxVoordeel) { tax = taxWithoutChildren - maxVoordeel; }
         }
        ...
        if (wealthPropNum > PARAMS.FR.IFI.DREMPEL_START) {
            ...
            for (const schijf of PARAMS.FR.IFI.SCHIJVEN) {
                 ...
                 if (taxedAmount <= currentGrens) break; 
            }
        }
    
        return { 
            bruto, tax: taxInFr, netto, wealthTax, 
            breakdown: { socialeLasten: totaleSocialeLasten + pfuSocialeLasten, aftrekCak, belastingKrediet, tax: Math.max(0, tax) + pfuTax, calculatedTaxIB: tax, parts, nettoInkomenUitNL, brutoInFR, brutoInkomenVoorNLBelasting, frStatePension: frStatePensionActive }
        };
    }
    ...
    function calculateBelgium(vals) {
        ...
        partners.forEach(p => {
            const salary = p.salary || 0, business = p.business || 0;
            const pensionPublic = p.pensionPublic || 0, pensionPrivate = p.pensionPrivate || 0;
            const lijfrente = p.lijfrente || 0, incomeWealth = p.incomeWealth || 0;
            ...
            if (business > 0) {
                let inkomenRestant = business, vorigeGrens = 0;
                P_BE.SOCIALE_LASTEN.ZELFSTANDIGE_SCHIJVEN.forEach(schijf => {
                    const currentGrens = Number(schijf.grens); 
            ...
            if (currentABP > 0) { nlPensioenVoorNLBelasting += currentABP; }
            ...
            } else { nlPensioenVoorBEBelasting += totaalOverigPensioen; }
        ...
        totaalBruto += nlPensioenVoorNLBelasting + nlPensioenVoorBEBelasting + totaalInkomenVermogen; // Inkomen vermogen ook in bruto
    
        const dividendDeel = totaalInkomenVermogen / 2, renteDeel = totaalInkomenVermogen / 2;
        ...
        const numChildren = vals.children || 0; 
        if (numChildren > 0) {
            const kindAftrek = P_BE.INKOMSTENBELASTING.VRIJSTELLING_PER_KIND; // Array [1e, 1e+2e, 1e+2e+3e]
            if (numChildren === 1) totaleVrijstelling += kindAftrek[0];
            else if (numChildren === 2) totaleVrijstelling += kindAftrek[1]; // Gebruik direct cumulatief bedrag
            else if (numChildren >= 3) {
                 const extraPerKind = kindAftrek[2] - kindAftrek[1]; // Bedrag voor 3e en volgende kinderen
                 totaleVrijstelling += kindAftrek[1] + (numChildren - 2) * extraPerKind; // Cumulatief t/m 2 + extra
            }
        }
        ...
        return { 
            bruto: totaalBruto, tax: totaleTax, netto: netto, wealthTax: wealthTax, 
            breakdown: { nettoInkomenUitNL, socialeLasten: totaleSocialeLasten, federaleBelasting, gemeentebelasting, roerendeVoorheffing: totaalRoerendeVoorheffing }
        };
    }
...
Huidig Bruto Inkomen: ${formatCurrency(compare.bruto)}
  ↳ Geschatte Lasten (IB + Zvw): ${formatCurrency(compare.tax)}
    (Toelichting: NL sociale lasten grotendeels in IB schijf 1. Zvw apart voor ondernemers.)
...
Vermogen:
- Jaarlijkse Aanslag: ${formatCurrency(compare.wealthTax)}
  (Toelichting: België kent geen algemene vermogensbelasting.)`;
        }
...
- Sociale lasten: -${formatCurrency(fr.breakdown.socialeLasten || 0)}
    (Toelichting: FR sociale lasten: ~9.1% pensioen, ~22% loon, ~21% winst, 17.2% vermogen.)
...
- Belastingkrediet Hulp aan Huis: +${formatCurrency(fr.breakdown.belastingKrediet || 0)}
  ↳ Berekende Inkomstenbelasting FR: ${formatCurrency(fr.breakdown.tax || 0)} 
     (Incl. PFU, kan negatief zijn: ${formatCurrency(fr.breakdown.calculatedTaxIB || 0)}) 

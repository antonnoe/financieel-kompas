document.addEventListener('DOMContentLoaded', () => {
    ...
    function getSimulationInfo(vals) {
        ...
        if (sY && sM) {
            simulatieDatum = new Date(sY, simMonth - 1, 15);
    ...
    function setupListeners() {
        ...
        // VERBETERDE KOPIEER FUNCTIE MET FALLBACK
        const cb = getEl('copy-btn');
        if (cb) {
            cb.addEventListener('click', async () => {
                const txt = outputs?.breakdown?.innerText || '';
                if (!txt || txt.includes("Welkom")) { alert("Genereer eerst een analyse."); return; }
                
                const success = () => { 
                    const orig = cb.textContent; cb.textContent = 'Gekopieerd!'; 
                    setTimeout(() => cb.textContent = orig, 2000); 
                };
                ...
                try {
                    ...
                    // Fallback voor iFrames zonder permissie
                    try {
                        const ta = document.createElement("textarea");
                        ta.value = txt; ta.style.position="fixed"; document.body.appendChild(ta);
                        ta.focus(); ta.select();
                        if (document.execCommand('copy')) success(); else throw new Error("Fallback failed");
        ...
        const ic = getEl('input-panel'); if (ic) { ic.addEventListener('input', (e) => { if (e.target.matches('input, select')) { if (e.target.id.includes('aow-years') || e.target.id.includes('fr-work-years') || e.target.id.includes('be-work-years')) { adjustWorkYears(e.target.id); } updateScenario(); } });} else { console.error("#input-panel missing!"); }
    }
    ...
    function calculateNetherlands(vals) {
        ...
        P.forEach((p, index)=>{
            ...
            const isWorking = simulatieDatum < aM;
...
4. Vermogen (Box 3):
   ...
   ↳ Aanslag: ${formatCurrency(compare.wealthTax)} (${((PARAMS.NL.BOX3.FORFAITAIR_RENDEMENT||0)*100).toFixed(2)}% fictief rend.)`;
            ...
            const nlTR=PARAMS.NL?.BOX1?.TARIEVEN_BOVEN_AOW?.[0]||0; const div=(1-nlTR); const bNP=div!==0?(compare.breakdown.nettoInkomenUitNL||0)/div:0;
...
* NL pensioen >€25k/jaar of overheidspensioen wordt in NL belast.`;
        ...
        const frSocLastenExclPFUBeLijfrente = fr.breakdown.frSocLastenInkomen || 0;
